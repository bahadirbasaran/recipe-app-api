...
class CommandTests(TestCase):
    ...
    def test_wait_for_db_ready(self):
        """Tests waiting for the database when the database is available.
           ...
           commands or call commands continue to execute.
        """
    ...
    def test_wait_for_db(self, ts):
        """Tests waiting for the db: Checks that the wait_for_db command tries
           ...
           waits for a second, and tries again. It is possible to remove this
           delay by using the patch decorator. The patch replaces the behavior
           of the time.sleep with a mock function that returns True.
        """
        with patch('django.db.utils.ConnectionHandler.__getitem__') as gi:
            # Set side effects to make it raise OperationalError in
            # the first five trials and succeed in the sixth trial.
